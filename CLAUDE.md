...
- **進捗グラフは「推定 1RM 換算」で統一**（2026-09-03 確定）: 実線 = 各セッションの est 1RM（使用重量 × Epley・種目別 rep 基準・RIR 含まぬ保守推定）、点線 = 生の 1RM ピーク。使用重量と 1RM を同一軸に混ぜない。progress-chart.html は DATA に使用重量を記入すれば描画時に自動換算される作り — 月次更新はこの方式を維持し、新しくグラフを作る場合も同じ換算規約に従う
